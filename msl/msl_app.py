    ...
    <div class="tab-content" id="tab-upload">
        <div class="main-grid">
            ...
            <div class="sidebar">
                ...
                <div class="card" id="uploadResultsCard">
                    <h3>📊 Top 3 Predictions</h3>
                    <div id="uploadTop3">
                        <div class="top3-row rank1"><span>---</span><span>--%</span></div>
                        <div class="top3-row rank2"><span>---</span><span>--%</span></div>
...
async function processVideo() {
    ...
    // More frames: 30 FPS for smoother processing
    const duration = uv.duration, fps = 30, totalFrames = Math.floor(duration * fps);
    ...
    vh.onResults(async r => {
        ...
        if ((r.leftHandLandmarks || r.rightHandLandmarks) && uk.length >= 30) {
            try {
                ...
                const data = await res.json();
                if (data.top3) {
                    predictions.push(data.top3[0]);
                    // Update live top 3 display on video
                    const best = data.top3[0];
                    ...
                    // Update sidebar top 3 in real-time
                    document.getElementById('uploadTop3').innerHTML = data.top3.map((t, i) =>
                        `<div class="top3-row rank${i+1}"><span>${t.gesture.replace('_', ' ')}</span><span><strong>${t.confidence}%</strong></span></div>`
                    ).join('');
                }
            } catch (e) {}
    ...
    for (let i = 0; i < totalFrames; i++) {
        ...
        await new Promise(r => uv.onseeked = r);
    ...
    if (predictions.length > 0) {
        ...
        document.getElementById('uploadStats').textContent = `${processed} frames analyzed, ${predictions.length} predictions made`;
    } else {
        document.getElementById('uploadResultGesture').textContent = 'No Hands';
        document.getElementById('uploadResultEnglish').textContent = 'No hands detected in video';
        document.getElementById('uploadResultConf').textContent = '';
    }
    document.getElementById('processBtn').disabled = false;
}
...
function resetUpload() {
    ...
    document.getElementById('uploadTop3').innerHTML = '<div class="top3-row rank1"><span>---</span><span>--%</span></div><div class="top3-row rank2"><span>---</span><span>--%</span></div><div class="top3-row rank3"><span>---</span><span>--%</span></div>';
